...
begin
	for i:=1 to eN do
  begin
    if enemys[i,1]=0 then goto down;
    if enemys[i,1]=px then
      if (enemys[i,2]+dis>py) and (enemys[i,2]<py) and (map[enemys[i,1],enemys[i,2]+1]=1) then//👉
    	begin
    ...
    if enemys[i,2]=py then
    if (enemys[i,1]+dis>px) and (enemys[i,1]<px) and (map[enemys[i,1]+1,enemys[i,2]]=1) then //👇
    	begin
      	wayG(enemys[i,1]*20,enemys[i,2]*20);
      	map[enemys[i,1],enemys[i,2]]:=1;
      	enemys[i,1]+=1;
    ...
    if enemys[i,1]<px then if enemys[i,1]+dis>px then
      ...
      begin//x1,y1
        if random(1)=0 then if map[enemys[i,1],enemys[i,2]+1]=1 then 
        begin
          wayG(enemys[i,1]*20,enemys[i,2]*20);
          map[enemys[i,1],enemys[i,2]]:=1;
          enemys[i,2]+=1;
...
procedure search_enemy (x:integer;y:integer);//надо переделать
begin
  var i:byte;
  i:=0;
  repeat
    ...
    map[enx,eny]:=1;
    wayG(enx*20,eny*20);
    if (enemys[i,1]=enx) and (enemys[i,2]=eny) then enemys[i,1]:=0 
  until(i=21);
end;
